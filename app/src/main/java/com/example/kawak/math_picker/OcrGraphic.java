...
public class OcrGraphic extends GraphicOverlay.Graphic {
    ...
    public void draw(Canvas canvas) {
        ...
        //RectF rect = new RectF(textBlock.getBoundingBox());
        RectF rect = new RectF(90, 275, 1000, 625);
        rect = translateRect(rect);
        ...
        for(Text currentText : textComponents) {
            ...
            if(left>rect.left && bottom < rect.bottom && currentText.getBoundingBox().top > rect.top){
                String text = currentText.getValue();
                if(text.contains("+")||text.contains("-")) {
                    //canvas.drawText(text, 200, 800, textPaint);
                    new DetectedVal().setValue(text);
                }
            }
        }
    }
